...
MODULE_VERSION = "metines_pipeline_visible_buttons_2026-07-16m_display_fix"
...
def _show_diagnostics(start_date: date, end_date: date):
    ...
    if files_df is not None and not files_df.empty:
        with st.expander("annual_report_files pavyzdžiai", expanded=False):
            ...
            st.dataframe(_safe_dataframe_for_streamlit(view[cols].head(50)), use_container_width=True, hide_index=True)

    if metrics_df is not None and not metrics_df.empty:
        with st.expander("annual_report_metrics žali duomenys", expanded=False):
            st.dataframe(_safe_dataframe_for_streamlit(metrics_df.head(200)), use_container_width=True, hide_index=True)



def _safe_dataframe_for_streamlit(df: pd.DataFrame) -> pd.DataFrame:
    """Paruošia DataFrame Streamlit/pyarrow rodymui.

    Supabase REST kartais grąžina stulpelius su mišriais tipais, pvz. tame pačiame
    stulpelyje būna skaičius, tekstas, dict/list arba None. Streamlit tada krenta su
    pyarrow.lib.ArrowInvalid. Diagnostikai ir lentelėms saugiau tokius object
    stulpelius paversti tekstu, o skaitinius palikti skaitiniais.
    """
        ...
        if pd.api.types.is_object_dtype(series):
            def clean_value(v):
                ...
                if isinstance(v, (dict, list, tuple, set)):
                    return str(v)
                return str(v)
            out[col] = series.map(clean_value)
    return out
...
def show_metines_page():
    ...
    st.info(f"AKTYVUS NAUJAS METINES MODULIS: {MODULE_VERSION}. Failas: {__file__}")
    st.markdown("<br>", unsafe_allow_html=True)

    st.warning(
        "DIAGNOSTIKA: jeigu matai šį bloką, užkrautas NAUJAS metines.py. "
        "Veiksmų mygtukai specialiai perkelti į pagrindinį puslapį, kad jų neuždengtų senas sidebar kodas."
    )

    with st.container(border=True):
        st.subheader("Metinių ataskaitų veiksmai")
        c1, c2, c3 = st.columns([1, 1, 1])
        with c1:
            start_date = st.date_input(
                "Metinės informacijos data nuo",
                value=date.today() - timedelta(days=730),
                key="metines_main_start_date",
            )
        with c2:
            end_date = st.date_input(
                "Metinės informacijos data iki",
                value=date.today(),
                key="metines_main_end_date",
            )
        with c3:
    ...
    with st.sidebar:
        ...
        st.caption(f"Aktyvi versija: {MODULE_VERSION}")
        st.caption("Veiksmų mygtukai yra pagrindiniame puslapyje, nebe šoninėje juostoje.")
    ...
    # Streamlit / pyarrow kartais nulūžta, kai DataFrame turi mišrių tipų
    # arba kai perduodamas pandas Styler. Todėl pagrindinę lentelę rodome
    # per saugų DataFrame, be Styler objekto.
    filtered_display = _safe_dataframe_for_streamlit(filtered)
